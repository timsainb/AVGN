...
def create_audio_envelope_waveform(
    waveform,
    wav_spectrogram,
    downsampled_rate,
    rate=44100,
    ...
    if signal == "waveform":
        downsampled = scipy.signal.resample(
            np.abs(waveform), int(float(len(waveform)) / rate * downsampled_rate)
        )
    ...
    if smoothing == "gaussian":
        ...
        return norm(
            ndimage.filters.gaussian_filter(
                np.array(downsampled, np.float32), gauss_sigma_f
            )
        )
    elif smoothing == "none":
...
def temporal_segmentation_int(
    vocal_envelope, fft_rate, segmentation_rate=1.0, threshold_max=0.3
):
...
def generate_fft_time_idx(vocalization, mel_spec, rate):
    ...
    fft_time_idx = np.arange(
        0, len(vocalization), float(len(vocalization)) / np.shape(mel_spec)[0]
    )
    fft_rate = 1.0 / (len(vocalization) / float(rate) / float(np.shape(mel_spec)[0]))
    return fft_time_idx, fft_rate


def compute_spec_and_env(
    vocalization,
    time_index,
    hparams,
    rate,
    ...
    while envelope_is_good == False:
        ...
        if (
            hparams_copy["min_level_db"] > spec_thresh_min
        ):  # end loop if thresh is too low
            return 0, 0, 0, 0, 0
        if verbose:
        ...
        if mel_filter:
            spec = norm_zero_one(
                sg.melspectrogram(vocalization, hparams_copy, _mel_basis)
            )
        else:
        ...
        if subtract_median:
            ...
            med = np.array(
                [
                    bn[np.argsort(freq)][-1]
                    for freq, bn in [
                        np.histogram(spec[i, :], bins=50) for i in range(len(spec))
                    ]
                ]
            )
            spec = spec - np.repeat(med, np.shape(spec)[1]).reshape(np.shape(spec))
        ...
        vocal_envelope = create_audio_envelope_waveform(
            np.abs(vocalization),
            wav_spectrogram=spec.T,
            downsampled_rate=fft_rate,
            rate=rate,
            gauss_sigma_s=gauss_sigma_s,
            smoothing=smoothing,
            signal=envelope_signal,
        )
        vocal_envelope = np.concatenate((vocal_envelope, np.zeros(1000)))[
            : np.shape(spec)[1]
        ]

        hparams_copy[
            "min_level_db"
        ] += spec_thresh_delta  # shift the noise threshold down
        ...
        if (
            np.max(cr[:, 1] - cr[:, 0]) > fft_rate * min_silence_for_spec
        ):  # needs .5 seconds of silence
            envelope_is_good = True

    spec = np.floor(norm_zero_one(spec_orig) * 255.0)
...
def cut_syllables(onsets, offsets, mel_spec, fft_time_idx, params):
    ...
    for i, on in enumerate(onsets):
        if offsets[i] > on + min_syll_len_s:  # if the syllable is sufficiently long
            ...
            for j in range(len(cur_seg_points) - 1):
                all_syllables.append(
                    mel_spec[:, cur_seg_points[j] : cur_seg_points[j + 1]]
                )
                all_syllables_time_idx.append(
                    fft_time_idx[cur_seg_points[j] : cur_seg_points[j + 1]]
                )
...
def threshold_syllables(
    ...
    """
    # Threshold time
    syll_lens = np.array([np.shape(i)[1] for i in all_syllables]) / float(fft_rate)
    good_sylls = (syll_lens >= min_syll_len_s) & (syll_lens <= max_syll_len_s)
...
def plot_seg_spec(
    ...
    cax = ax[0].matshow(
        mel_spec, interpolation=None, aspect="auto", origin="lower"
    )  # cmap=plt.cm.afmhot
    for i in all_seg_points:
        ax[0].axvline(i, color="w", linestyle="-", lw=3, alpha=0.75)  # , ymax=.25)
    cax = ax[1].matshow(
        mel_spec, interpolation=None, aspect="auto", origin="lower"
    )  # cmap=plt.cm.afmhot
        ...
        if time_len > ebr_max:
            ...
            if cur_thresh < max_thresh:
                while cur_thresh < max_thresh:
                    new_onsets, new_offsets = detect_onsets_offsets(
                        norm(vocal_envelope[onset:offset]),
...
def resize_spectrograms(
    all_syllables, max_size, resize_samp_fr, fft_rate, n_freq, pad_length
):
...
def plot_pipeline(
    ...
    ax[2].matshow(
        ...
        cmap=plt.cm.viridis,
        origin="lower",
        extent=[0, len(data) / rate, 0, rate / 2],
    )

    for on, off in zip(
        np.array(onsets) / float(len(wav_spectrogram)) * (len(data) / rate),
        np.array(offsets) / float(len(wav_spectrogram)) * (len(data) / rate),
    ):
        ax[2].axvline(on, color="white", linestyle="-", lw=1, alpha=0.75)
        ax[2].axvline(off, color="white", linestyle="-", lw=1, alpha=0.75)
    ...
    for loopi, mldb in enumerate(
        tqdm(
            np.arange(
                ...
                hparams["delta_min_level_db"],
            ),
            leave=False,
            disable=(not verbose),
        )
    ):
        hparams_copy["min_level_db"] = mldb
        # get the spectrogram
        spec = norm_zero_one(sg._normalize(spec_orig, hparams_copy))
        # subtract the median
        if subtract_median:
        ...
        if len(cr_on) > 0:
            if verbose:
                print("amount of silence", np.max(cr_on[:, 1] - cr_on[:, 0]) / fft_rate)
            if verbose:
                print(
                    "longest vocalization",
                    np.max((cr_off[:, 1] - cr_off[:, 0]) / fft_rate),
                )

            if (
                np.max(cr_on[:, 1] - cr_on[:, 0]) / fft_rate
                > hparams["min_silence_for_spec"]
            ):
                if (
                    np.max((cr_off[:, 1] - cr_off[:, 0]) / fft_rate)
                    < hparams["max_vocal_for_spec"]
                ):
                    envelope_is_good = True
                    break
        hparams_copy["min_level_db"] += hparams[
            "spec_thresh_delta"
        ]  # shift the noise threshold down

    if envelope_is_good == False:
...
def process_bout_cdp(wav_file, _mel_basis, hparams, submode=True, visualize=False):
    ...
    if visualize:
        print(wav_file)
    csv_loc = "/".join(
        wav_file.split("/")[:-2] + ["csv"] + [wav_file.split("/")[-1][:-4] + ".csv"]
    )
    try:
    ...
    # Compute spectrogram
    spec, vocal_envelope, cur_spec_thresh, fft_time_idx, fft_rate = compute_spec_and_env_cdp(
        vocalization,
        start_time,
        hparams,
        rate,
    ...
    onsets, offsets = detect_onsets_offsets(
        vocal_envelope, threshold=silence_threshold, min_distance=0.0
    )
    ...
    syllable_lengths = [
        np.shape(all_syllables[i])[1] / fft_rate for i in range(len(all_syllables))
    ]
    ...
    all_syllables, all_syllables_time_idx, syll_start = threshold_syllables(
        ...
        all_syllables_time_idx,
        syll_start,
        min_syll_len_s=hparams["min_syll_len_s"],
        max_syll_len_s=hparams["max_syll_len_s"],
        fft_rate=fft_rate,
        power_thresh=hparams["power_thresh"],
        visual=False,
        max_vis=20,
    )
    ...
    syll_start_dt = [
        dati.strftime("%d/%m/%y %H:%M:%S.%f")
        for dati in [start_time + timedelta(seconds=st / rate) for st in syll_start]
    ]  # datetime info for when the syllable occurs
    ...
    # resize spectrogram
    all_syllables = resize_spectrograms(
        all_syllables,
        max_size=syll_size,
        resize_samp_fr=np.floor(syll_size / max_syll_len_s),
        fft_rate=fft_rate,
        n_freq=syll_size,
    ...
    # 0 pade
    all_syllables = np.array([pad_spectrogram(i, syll_size) for i in all_syllables])
...
def process_bout(wav_file, _mel_basis, hparams, submode=True, visualize=False):
    ...
    spec, vocal_envelope, cur_spec_thresh, fft_time_idx, fft_rate = compute_spec_and_env(
        (vocalization / 32768.0).astype("float32"),
        start_time,
        hparams,
        rate,
        _mel_basis,
    ...
    all_syllables = resize_spectrograms(
        all_syllables,
        max_size=max_size_syll,
        resize_samp_fr=resize_samp_fr,
        fft_rate=fft_rate,
        n_freq=num_freq_final,
...
def save_dataset(
    ...
    with h5py.File(location, "w") as f:
        ...
        dset_spec = f.create_dataset(
            "spectrograms", np.shape(all_bird_syll), dtype="uint8", data=all_bird_syll
        )
        dset_start = f.create_dataset(
            "start", (len(starting_times), 1), dtype=dt, data=starting_times
        )
        dset_wav_file = f.create_dataset(
            "wav_file", (len(wav_file), 1), dtype=dt, data=wav_file
        )
        dset_syll_start_rel_to_wav = f.create_dataset(
            "syll_start_rel_wav",
            np.shape(syll_start_rel_to_wav),
            dtype="float32",
            data=syll_start_rel_to_wav,
        )
        dset_lengths = f.create_dataset(
            "lengths", np.shape(lengths), dtype="float32", data=lengths
        )
...
def iterateCreateSyllSpec(data, _mel_basis, hparams, pct_fail, power_thresh):
    ...
    while (cur_spec_thresh < thresh_min) & (wav_spectrogram is None):
        ...
        wav_spectrogram = norm_zero_one(
            sg.melspectrogram(data, hparams_copy, _mel_basis)
        )
        wav_spectrogram[wav_spectrogram < power_thresh] = 0
        # print(np.shape(wav_spectrogram))
        pct_sil = np.sum(wav_spectrogram < power_thresh) / np.prod(
            np.shape(wav_spectrogram)
        )

        plt.matshow(wav_spectrogram)
        plt.show()
        print(pct_sil, pct_fail)
        if pct_sil < pct_fail:
...
def getSyllsFromWav(row, _mel_basis, wav_time, hparams):
    ...
    data = butter_bandpass_filter(
        data, hparams["lowcut"], hparams["highcut"], rate, order=3
    ).astype("int16")
    syll_info = []
    # go through tier
    for (syll_start, syll_len, syll_sym) in zip(
        row.NotePositions, row.NoteLengths, row.NoteLabels
    ):
        ...
        mel_spec = iterateCreateSyllSpec(
            data=data[syll_start:syll_stop].astype("float64"),
            _mel_basis=_mel_basis,
            hparams=hparams,
            pct_fail=hparams["pct_fail"],
            power_thresh=hparams["power_thresh"],
        )
        if mel_spec is None:
        ...
        syll_info.append(
            [
                row.WavLoc,
                mel_spec,
                (wav_time + timedelta(seconds=syll_start / rate)).strftime(
                    "%d/%m/%y %H:%M:%S.%f"
                ),
                syll_start,
                (syll_stop - syll_start) / rate,
                syll_sym,
            ]
        )
    return syll_info
