...
def sequential_mutual_information(sequences, distances, n_jobs = 1, verbosity = 5, n_shuff_repeats=1):
    """
    ...
    with Parallel(n_jobs=n_jobs, verbose=verbosity) as parallel:
        ...
        shuff_MI = np.array([parallel(delayed(MI_from_distributions)(shuffled_sequences, dist) for dist_i, dist in enumerate(tqdm(distances, leave=False))) for i in np.arange(n_shuff_repeats)])
        shuff_MI = np.mean(shuff_MI, axis = 0)
