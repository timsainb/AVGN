...
def residuals(y_true, y_model,x, logscaled=False):
    if logscaled:
        return np.abs(np.log(y_true)-np.log(y_model))*(1/(1+np.log(x)))
    else:
...
def concat_decay(p, x):
    return p['aa']*(x**(p['pb'])) + p['a']*(np.exp(-x*p['tau'])) + p['c_concat']
...
def fit_models(sig, distances,parameters, fit):
    ...
    results_power = results_power_min.minimize(method='least-squares')

    # exponential
    results_exp_min = lmfit.Minimizer(exp_decay_res, p_exp, fcn_args = (distances,sig, fit), nan_policy='omit')
    results_exp = results_exp_min.minimize(method='least-squares')

    # concatenative
    results_concat_min = lmfit.Minimizer(concat_decay_res, p_concat, fcn_args = (distances,sig, fit), nan_policy='omit')
    results_concat = results_concat_min.minimize(method='least-squares')


    return results_power, results_exp, results_concat


def plot_MI(sig, distances, title, ax, line=True):
    if line:
        ax.plot(distances, sig, alpha = 1, lw=3, color='k')
    else:
        ax.scatter(distances, sig)
    ax.plot(distances, sig, alpha = 0)
